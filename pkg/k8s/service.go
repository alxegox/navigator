...
func (n QualifiedName) WholeMeshKey() QualifiedName {
	wm := n
	wm.Namespace = WholeMeshName
	return wm
}
...
func (s *Service) UpdateConfig(config NexusConfig) (updated bool) {
	if config.CookieAffinity == s.CookieAffinity {
		return false
	}
	s.NexusConfig = config
	return true
}
