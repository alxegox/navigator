...
func TestEndpointEventHandler(t *testing.T) {
	...
	epHandler := NewEndpointEventHandler(logger, observability.DefaultMetrics, fakeCache, fakeServiceNotifier, fakeCanary, clusterID)
...
func TestServiceEventHandler(t *testing.T) {
	...
	fakeCache := &MockCache{}
	fakeCache.
		On("UpdateService", svc1.Namespace, svc1.Name, clusterID, ip1, k8sPorts, nexusConfig).
		Return([]QualifiedName{svc1})
	fakeCache.
		On("UpdateService", svc1.Namespace, svc1.Name, clusterID, ip2, k8sPorts, nexusConfig).
	...
	fakeNexusCache.
		On("GetConfigByService", svc1.Namespace, svc1.Name).
		Return(nexusConfig)
...
func TestCanaryReleaseEventHandler(t *testing.T) {
	...
	epHandler := NewEndpointEventHandler(logger, observability.DefaultMetrics, fakeCache, fakeServiceNotifier, fakeCanary, clusterID)
	canaryHandler := NewCanaryReleaseEventHandler(logger, observability.DefaultMetrics, fakeCache, fakeCanary, clusterID, fakeEndpointsLister, epHandler)
	observability.DefaultMetrics.Reset()
...
func TestNexusEventHandler(t *testing.T) {
	...
	fakeCache := &MockCache{}
	fakeCache.
		On("UpdateConfig", []QualifiedName{}, nexusConfig).
		Return([]QualifiedName(nil))

	fakeServiceNotifier := &MockServiceNotifier{}
	fakeServiceNotifier.On("NotifyServicesUpdated", []QualifiedName{})

	fakeNexus := &MockNexusCache{}
	fakeNexus.On("Update", clusterID, nexus1).Return([]string{app1})
	fakeNexus.On("Update", clusterID, nexus2).Return([]string{app1})
	fakeNexus.On("Delete", clusterID, nexus2).Return([]string{app1})
	fakeNexus.On("GetNexus", app1).Return(NewNexus(app1, []QualifiedName(nil), []QualifiedName(nil), []InboundPort(nil)))

	// check mock
	fakeNexus.On("GetSnapshotByAppName").Return(map[string]Nexus{})
	fakeNexus.On("GetAppNamesSnapshotByService").Return(map[QualifiedName][]string{})
	fakeNexus.GetSnapshotByAppName([]string{})
	fakeNexus.GetAppNamesSnapshotByService([]QualifiedName{})
...
func TestIngressEventHandler(t *testing.T) {
	...
	fakeNexus := &MockNexusCache{}
	fakeNexus.On("Update", clusterID, NewVirtualNexus(app1, app1, app1, services)).Return([]string{app1})
...
func TestGatewayEventHandler(t *testing.T) {
	...
	fakeNexus := &MockNexusCache{}
	fakeNexus.On("Update", clusterID, NewVirtualNexus(app1, app1, app1, services)).Return([]string{app1})
	fakeNexus.On("Delete", clusterID, NewVirtualNexus(app1, app1, app1, nil)).Return([]string{app1})
	fakeGateway := &MockGatewayCache{}
