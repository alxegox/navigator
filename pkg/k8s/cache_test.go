...
func TestCacheService(t *testing.T) {
	...
	c := NewCache(logger, observability.DefaultMetrics)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			switch tt.op {
			case update:
				got = tt.cache.UpdateService(tt.args.namespace, tt.args.name, tt.args.clusterID, tt.args.ClusterIP, tt.args.ports, tt.args.config)
			case remove:
...
func TestCacheMetrics(t *testing.T) {
	...
	updateServices := func(c Cache, services []updateService) {
		for i, us := range services {
			c.UpdateService(us.service, us.service, us.clusterID, fmt.Sprintf("10.0.0.%d", i), nil, NexusConfig{})
		}
	}
	...
	for n, tt := range tests {
		...
		servicesResult := testutil.CollectAndCompare(
			observability.DefaultMetrics.ServiceCount,
		...
		backendsResult := testutil.CollectAndCompare(
			observability.DefaultMetrics.BackendsCount,
