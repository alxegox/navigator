...
func (mc *MockCache) UpdateService(namespace, name, clusterID, clusterIP string, ports []Port, config NexusConfig) []QualifiedName {
	args := mc.Called(namespace, name, clusterID, clusterIP, ports, config)
...
func (mc *MockCache) UpdateConfig(services []QualifiedName, config NexusConfig) (updatedServiceKeys []QualifiedName) {
	args := mc.Called(services, config)
	return args.Get(0).([]QualifiedName)
}
...
func (mnc *MockNexusCache) Delete(cluster string, nexusResource *navigatorV1.Nexus) []string {
	args := mnc.Called(cluster, nexusResource)
	return args.Get(0).([]string)
}

func (mnc *MockNexusCache) Update(cluster string, nexusResource *navigatorV1.Nexus) []string {
	args := mnc.Called(cluster, nexusResource)
	return args.Get(0).([]string)
}

func (mnc *MockNexusCache) GetSnapshotByAppName(appNames []string) (nexusesByAppName map[string]Nexus) {
	args := mnc.Called()
	return args.Get(0).(map[string]Nexus)

}
func (mnc *MockNexusCache) GetAppNamesSnapshotByService(servicesInvolved []QualifiedName) (appNamesByService map[QualifiedName][]string) {
	args := mnc.Called()
	return args.Get(0).(map[QualifiedName][]string)

}

func (mnc *MockNexusCache) GetConfigByService(namespace, name string) NexusConfig {
	args := mnc.Called(namespace, name)
	return args.Get(0).(NexusConfig)
}

func (mnc *MockNexusCache) GetNexus(appName string) Nexus {
	args := mnc.Called(appName)
	return args.Get(0).(Nexus)
}
