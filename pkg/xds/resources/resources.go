...
type BranchedResourceCache struct {
	...
	// cache contains resources for each branch
	cache map[string]*resourceCache

	staticResources []NamedProtoMessage
	// for now dynamicResources are used for inbound resources
	// if need arises for other types, SetDynamicResources should be reimplemented with updated/deleted logic
	// and use map to simplify delete logic
	dynamicResources []NamedProtoMessage
...
func (c *BranchedResourceCache) Get(branch string) (grpc.Resource, bool) {
	...
	if !ok {
		...
		r = NewResourceCache(c.typeURL, initResources)
		c.cache[branch] = r
	}
	return r, true
}
...
func (c *BranchedResourceCache) SetDynamicResources(dynamicResources []NamedProtoMessage) {
	c.mu.Lock()
	c.mu.Unlock()

	c.dynamicResources = dynamicResources
	for _, resource := range c.cache {
		resource.UpdateResources(c.dynamicResources)
	}
}
...
func (c *BranchedResourceCache) renewResourceCache(branches ...string) {
	for _, branch := range branches {
		if resources, ok := c.cache[branch]; ok {
			resources.Flush()
			initResources := append(c.staticResources, c.dynamicResources...)
			resources.UpdateResources(initResources)
		}
	}
}
