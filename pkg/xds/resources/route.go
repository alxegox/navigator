...
func (c *RouteCache) UpdateInboundPorts(ports []k8s.InboundPort) {
	// do nothing
}
	...
	for _, ingress := range updated {
		...
		for _, rule := range ingress.Rules {
			...
			for _, path := range rule.Paths {
				...
				routes[ingress.ClusterID][rule.Host][path.Path] = getRoute(c.AppName, path.Path, path.Rewrite, routeTo, hashPolicy)
			}
		}
	}
...
func (c *RouteCache) getServiceRoutes(storage map[string][]NamedProtoMessage, service *k8s.Service) {
	for _, p := range service.Ports {
		...
		for _, clusterIP := range service.ClusterIPs {
			name := ListenerNameFromService(service.Namespace, service.Name, clusterIP.IP, p.Port)
			clusters := weightedClusters(service.Namespace, service.Name, p.Name)
			routeConf := routeServiceConfiguration(name, c.AppName, service.CookieAffinity, clusters)
...
func routeServiceConfiguration(name, appName, cookieAffinity string, clusters *route.WeightedCluster) *api.RouteConfiguration {

	var hashPolicy []*route.RouteAction_HashPolicy
	if cookieAffinity != "" {
		hashPolicy = routeHashPolicy(cookieAffinity)
	}
...
func routeInboundConfiguration(name string, clusterName string) *api.RouteConfiguration {
	return &api.RouteConfiguration{
		Name: name,
		VirtualHosts: []*route.VirtualHost{{
			Name:    name,
...
func getRoute(appName, pathPrefix, prefixRewrite string, weightedCluster *route.WeightedCluster, hashPolicy []*route.RouteAction_HashPolicy) *route.Route {
	return &route.Route{
		RequestHeadersToAdd: []*core.HeaderValueOption{{
			Header: &core.HeaderValue{
				Key:   "X-Source",
