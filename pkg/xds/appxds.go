...
type ClusterResource interface {
	...
	UpdateInboundPorts(ports []k8s.InboundPort)
...
func NewAppXDS(appName string, logger logrus.FieldLogger, resOpts []resources.FuncOpt) *AppXDS {
	resOpts = append(resOpts, resources.WithAppName(appName))
...
func (x *AppXDS) Update(services []*k8s.Service) {
	...
	updated, deleted := servicesDiff(x.prevServices, services)
	for _, resource := range x.clusterResources {
		resource.UpdateServices(updated, deleted)
	}
...
func (x *AppXDS) InboundPortsUpdate(ports []k8s.InboundPort) {
	x.mu.Lock()
	defer x.mu.Unlock()

	for _, resource := range x.clusterResources {
		resource.UpdateInboundPorts(ports)
	}
}
