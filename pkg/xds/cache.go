...
type nexusCache interface {
	// GetSnapshotByAppName returns nexuses indexed by app Id
	GetSnapshotByAppName(appNames []string) (nexusesByAppName map[string]k8s.Nexus)
	// GetAppNamesSnapshotByService returns app names indexed by services which are interested in them
	GetAppNamesSnapshotByService(servicesInvolved []k8s.QualifiedName) (appNamesByService map[k8s.QualifiedName][]string)
}
...
func (c *Cache) NotifyGatewayUpdated(updatedGateway *k8s.Gateway, deleted bool) {
	...
	if _, ok := c.appXDSs[updatedGateway.Name]; !ok {
		c.appXDSs[updatedGateway.Name] = NewAppXDS(updatedGateway.Name, c.logger, c.resOpts)
...
func (c *Cache) NotifyIngressUpdated(ingress *k8s.Ingress) {
	...
	for _, gw := range gateways {
		if _, ok := c.appXDSs[gw.Name]; !ok {
			c.appXDSs[gw.Name] = NewAppXDS(gw.Name, c.logger, c.resOpts)
		}
...
func (c *Cache) NotifyNexusesUpdated(updatedAppNames []string, isGateway bool) {
	nexuses := c.nexusCache.GetSnapshotByAppName(updatedAppNames)
	...
	for _, appName := range updatedAppNames {
		if _, ok := c.appXDSs[appName]; !ok {
			c.appXDSs[appName] = NewAppXDS(appName, c.logger, c.resOpts)
		}
		nexus := nexuses[appName]
		c.appXDSs[appName].InboundPortsUpdate(nexus.InboundPorts)
...
func (c *Cache) NotifyServicesUpdated(updatedServiceKeys []k8s.QualifiedName) {
	...
	// match all services with whole mesh
	allNeededServices := []k8s.QualifiedName{{Namespace: k8s.WholeMeshName, Name: ""}}
	// add exact services and whole namespaced
	for _, serviceKey := range updatedServiceKeys {
		// match all services with whole namespace
		allNeededServices = append(allNeededServices, serviceKey.WholeNamespaceKey())
		// match exact services
		allNeededServices = append(allNeededServices, serviceKey)
	}

	appNamesByService := c.nexusCache.GetAppNamesSnapshotByService(allNeededServices)

	appNames := make(map[string]struct{})
	for _, appNamesForService := range appNamesByService {
		for _, appName := range appNamesForService {
			appNames[appName] = struct{}{}
		}
	}

	var appNamesStrs []string
	for appName := range appNames {
		appNamesStrs = append(appNamesStrs, appName)
	}

	nexuses := c.nexusCache.GetSnapshotByAppName(appNamesStrs)

	c.mu.Lock()
	defer c.mu.Unlock()

	for _, appName := range appNamesStrs {
		if x, ok := c.appXDSs[appName]; ok {
			...
			c.logger.Errorf(
				"Inconsistent envoy cache: there is no such appName '%s' in c.appXDSs",
				appName,
			)
		}
	}
...
func (c *Cache) Get(appName, typeURL, clusterID string) (result grpc.Resource, ok bool) {
	...
	if !ok {
		appXDS = NewAppXDS(appName, c.logger, c.resOpts)
