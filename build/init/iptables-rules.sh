...
# ports for envoy inbound
ENVOY_INBOUND_INTERCEPT_PORTS=${ENVOY_INBOUND_INTERCEPT_PORTS:-" "}
...
ENVOY_SIDECAR_INBOUND_PORT=${ENVOY_SIDECAR_INBOUND_PORT:-15006}
...
# if netra and envoy inbound rules are applied
if ! [[ "${ENVOY_INBOUND_INTERCEPT_PORTS}" == " " ]] && ! [[ "${INBOUND_INTERCEPT_PORTS}" == " " ]]; then
    if [[ "${ENVOY_INBOUND_INTERCEPT_PORTS}" == "*" ]]; then
        iptables -t nat -A OUTPUT -d 127.0.0.1/32 -o lo -p tcp -m owner --uid ${NETRA_SIDECAR_USER_ID} -j REDIRECT --to-ports ${ENVOY_SIDECAR_INBOUND_PORT}
    else
        for port in ${ENVOY_INBOUND_INTERCEPT_PORTS}; do
            iptables -t nat -A OUTPUT -d 127.0.0.1/32 -o lo -p tcp -m owner --uid ${NETRA_SIDECAR_USER_ID} --dport ${port} -j REDIRECT --to-ports ${ENVOY_SIDECAR_INBOUND_PORT}
        done
    fi
fi
...
if [[ "${INBOUND_INTERCEPT_PORTS}" == " " ]]; then
    if [[ "${ENVOY_INBOUND_INTERCEPT_PORTS}" == "*" ]]; then
        iptables -t nat -A PREROUTING -p tcp -m tcp -j REDIRECT --to-ports ${ENVOY_SIDECAR_INBOUND_PORT}
    else
        for port in ${ENVOY_INBOUND_INTERCEPT_PORTS}; do
            iptables -t nat -A PREROUTING -p tcp -m tcp --dport ${port} -j REDIRECT --to-ports ${ENVOY_SIDECAR_INBOUND_PORT} || true
        done
    fi
fi