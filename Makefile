...
NAVIGATOR_IMAGE?=registry.k.alxegox.dev/avito/navigator
NAVIGATOR_IMAGE_INIT?=registry.k.alxegox.dev/avito/navigator-init
NAVIGATOR_IMAGE_SIDECAR?=registry.k.alxegox.dev/avito/navigator-sidecar
VERSION?=1.6.0
...
.PHONY: test-e2e
test-e2e:
	go test -timeout 500m -count=1 -mod vendor  -v -tags e2e_test github.com/alxegox/navigator/tests/e2e/tests ${FLAGS}

.PHONY: test-e2e-locality-disabled
test-e2e-locality-disabled:
	go test -timeout 50m -count=1 -mod vendor  -v -tags e2e_test github.com/alxegox/navigator/tests/e2e/tests --enable-locality=false ${FLAGS}
...
.PHONY: test-ci
test-ci:
	docker run -v $$(pwd):/app -w /app golang:1.14 make test-unit
