...
VERSION=$(go list -mod=mod -m all | grep k8s.io/code-generator | rev | cut -d"-" -f1 | cut -d" " -f1 | rev)
TMP_DIR=$(mktemp -d)
GOPATH=$(mktemp -d)
...
${TMP_DIR}/generate-groups.sh \
  all \
  github.com/alxegox/navigator/pkg/apis/generated \
  github.com/alxegox/navigator/pkg/apis \
  navigator:v1 \
  $@
...
mv "$GOPATH/src/github.com/alxegox/navigator/pkg/apis/generated" "$GENERATED_DIR"
cp -r "$GOPATH/src/github.com/alxegox/navigator/pkg/apis/" "$APIS_DIR"
