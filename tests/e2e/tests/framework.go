...
func UpdateDiscovery(clusterList []string, discoveries []Discovery) error {
	var deps []*v12.Nexus
	...
	for srcAppName, destinations := range *d {
		dep := &v12.Nexus{
			...
			Spec: v12.NexusSpec{
				...
				InboundPorts: []v12.InboundPort{{Name: "http", Port: 80}},
			},
		}
...
func RemoveNexus(clusterName, appName, nexusName string) error {
	cs, err := getExtClientsetByClusterName(clusterName)
	if err != nil {
		return err
	}

	err = cs.NavigatorV1().Nexuses(appName).Delete(nexusName, nil)
	if err != nil {
		return fmt.Errorf("failed to remove nexus: %s", err.Error())
	}

	return nil
}

func RemoveNamespace(clusterName, nsName string) error {
	cs, err := getClientsetByClusterName(clusterName)
	if err != nil {
		return err
	}

	err = cs.CoreV1().Namespaces().Delete(nsName, nil)
	if err != nil {
		return fmt.Errorf("failed to remove namespace: %s", err.Error())
	}
