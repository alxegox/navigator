...
import (
	...
	v12 "github.com/alxegox/navigator/pkg/apis/navigator/v1"
)
...
func TestAddNexus(t *testing.T) {
	...
	for i, c := range cases {
		...
		err := UpdateDiscovery(getClusterList(clusterCount), discoveries)
		if err != nil {
...
func TestUpdateNexus(t *testing.T) {
	...
	for i, c := range cases {
		...
		err := UpdateDiscovery(getClusterList(clusterCount), []Discovery{discovery})
		if err != nil {
			t.Fatalf("Failed to update discovery in case #%d: %s", i, err.Error())
		}
...
func TestDeleteNexus(t *testing.T) {
	cases := [][]struct {
		srcAppName, clusterName string
	}{
		{
			{clusterName: getClusterName(1), srcAppName: getAppName(1)},
			{clusterName: getClusterName(2), srcAppName: getAppName(1)},
		},
		{
			{clusterName: getClusterName(1), srcAppName: getAppName(1)},
		},
		{
			{clusterName: getClusterName(1), srcAppName: getAppName(1)},
			{clusterName: getClusterName(1), srcAppName: getAppName(2)},
		},
	}
	...
	for i, c := range cases {
		...
		initialDiscovery := SetupTest(t, appCount)
		discoveryByCluster := map[string]Discovery{}
		for _, clusterName := range getClusterList(clusterCount) {
			discoveryByCluster[clusterName] = initialDiscovery.Copy()
		}

		toRemoveSourceByAppName := map[string][]string{}
		for _, rule := range c {
			toRemoveSourceByAppName[rule.srcAppName] = append(toRemoveSourceByAppName[rule.srcAppName], rule.clusterName)
			if len(toRemoveSourceByAppName[rule.srcAppName]) == clusterCount {
				for _, clusterName := range getClusterList(clusterCount) {
					discoveryByCluster[clusterName].RemoveSource(rule.srcAppName)
				}
			}

			err := RemoveNexus(rule.clusterName, NavigatorNamespace, fmt.Sprintf("%s-v%d", rule.srcAppName, 0))
			if err != nil {
				t.Fatalf("Failed to update discovery in case #%d: %s", i, err.Error())
			}
		}

		for clusterName, discovery := range discoveryByCluster {
			if err := checkClusterCoherence(t, clusterName, discovery, appCount); err != nil {
				t.Errorf("Failed case #%d check full coherence: %s", i, err.Error())
			}
		}
	}
...
func TestDeleteNamespace(t *testing.T) {
	cases := [][]struct {
		...
		{
			{clusterName: getClusterName(1), namespace: getAppName(1)},
			{clusterName: getClusterName(2), namespace: getAppName(1)},
		},
		{
			{clusterName: getClusterName(1), namespace: getAppName(1)},
		},
		{
			{clusterName: getClusterName(1), namespace: getAppName(1)},
			{clusterName: getClusterName(1), namespace: getAppName(2)},
		},
	}
	...
	for i, c := range cases {
		...
		for _, rule := range c {
			discoveryByCluster[rule.clusterName].RemoveSource(rule.namespace)
			for _, s := range getAppList(appCount) {
				discoveryByCluster[rule.clusterName].RemoveDestination(s, rule.namespace)
			}
			err := RemoveNamespace(rule.clusterName, rule.namespace)
		...
		for clusterName, discovery := range discoveryByCluster {
			...
			for _, appName := range getAppList(appCount) {
				skip := false
				for _, rule := range c {
					if rule.clusterName == clusterName && rule.namespace == appName {
						skip = true
						break
					}
				}
...
func checkClusterCoherence(t *testing.T, clusterName string, discovery Discovery, appCount int) error {
	wantPodWeights, err := getWantPodWeights(
		clusterName,
		getAppList(appCount),
		defaultComponentList,
